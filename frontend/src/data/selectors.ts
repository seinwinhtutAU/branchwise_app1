  ...
  return fx.packageItems
    ...
    .map((pki) => {
      ...
      return {
        ...
        product: getProduct(purchaseItem.productId)!,
...
export function getOrderItemsWithFulfillment(orderId: string): OrderItemFulfillment[] {
  ...
  const orderDeliveryIds = fx.deliveries.filter((d) => d.orderId === orderId).map((d) => d.deliveryId);
  ...
  return items.map((item) => {
    ...
    const delivered = fx.deliveryItems
      .filter((di) => orderDeliveryIds.includes(di.deliveryId) && di.productId === item.productId)
      .reduce((sum, di) => sum + di.quantity, 0);

    return { item, product, ordered: item.quantity, purchased, delivered };
  });
}
